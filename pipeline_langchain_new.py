...
from coref_langchain import find_entity_mentions
from entity_extraction_langchain_new import REFERRING_EXPRESSIONS, extract_entities_from_post
from text_utils import preprocess_text, split_into_sentences

load_dotenv()

REFERRING_EXPRESSION_SET = {ref.lower() for ref in REFERRING_EXPRESSIONS}
...
def labels_to_aspects(label_out: Any) -> List[Tuple[str, str, str]]:
    ...
    aspect_label: "Aspect A", "Aspect B", or "Aspect C"
    ...
    if isinstance(label_out, dict):
        ...
        # Aspect C
        c_answer = str(label_out.get("C", "")).strip().lower()
        if c_answer == "yes":
            c_sentiment = _norm_sentiment(label_out.get("C_sentiment", "None"))
            if c_sentiment not in {"Positive", "Negative", "Neutral"}:
                c_sentiment = "Neutral"
        else:
            c_sentiment = "None"

        c_presence = "Positive" if c_answer == "yes" else "Negative"
        aspects.append(("Aspect C", c_sentiment, c_presence))
...
def _build_explicit_mentions(
    ...
    for occurrence_index, (entity, start, end) in enumerate(explicit_spans, 1):
        ...
        mention_type = "referring_expression" if mention_text.lower() in REFERRING_EXPRESSION_SET else "explicit_text"
        mentions.append(
            {
...
def run_one_post(
    ...
    report: Dict[str, Any] = {
        ...
        "entity_extraction": {},
        "coreference": {
            "mode": "disabled_in_new_pipeline",
            "note": "Pronouns/referring expressions are tagged directly as entity mentions.",
            "sentences": [],
        },
    ...
    if debug:
        ...
        print("4) COREFERENCE")
        print("==============================")
        print("Coreference LLM is disabled in pipeline_new.")
        print("Pronouns/referring expressions are classified directly as <E>...</E> mentions.")
