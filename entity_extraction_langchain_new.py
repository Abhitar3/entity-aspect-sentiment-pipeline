...
def _find_referring_expression_surfaces(post_text: str) -> List[str]:
    pattern = "|".join(re.escape(ref) for ref in REFERRING_EXPRESSIONS)
    surfaces: List[str] = []
    for match in re.finditer(rf"(?<!\w)({pattern})(?!\w)", post_text, flags=re.IGNORECASE):
        surfaces.append(match.group(0))
    return _dedup_exact(surfaces)
...
def extract_entities_from_post(
    ...
    referring_expressions = _find_referring_expression_surfaces(post_clean)
    cleaned = _dedup_exact(cleaned + referring_expressions)
    evidence = {entity: ["context:post"] for entity in cleaned}

    if debug:
        print("\n[ENTITY EXTRACTION]")
        print("Raw LLM JSON:", out)
        print("Referring expressions added:", referring_expressions)
        print("After filters + referring expressions:", cleaned)
